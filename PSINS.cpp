...
void CIMU::Update(const CVect3 *wm, const CVect3 *vm, int nSamples)
{
	...
	{
		if(prefirst==1) {wm_1=wm[0]; vm_1=vm[0]; prefirst=0;}//保存前一次角/速度增量采样
	...
	wmm += wm[i];
	vmm += vm[i];
	phim = wmm + cm*wm[i];		//补偿后等效旋转矢量、
	dvbm = vmm + 1.0/2*wmm*vmm + (cm*vm[i]+sm*wm[i]); // 补偿后比力增量、
}
...
void CSINS::Update(const CVect3 *wm, const CVect3 *vm, int nSamples, double ts)
{
	...
	wib = imu.phim/nts;	//载体坐标系角速度
	fb = imu.dvbm/nts;	//载体坐标系加速度
...
/******************************  File Read or Write *********************************/
CFileRdWt::CFileRdWt(const char *fname, int columns)
{
...
{
/*  Matlab
avp = randn(9,1);  wvm = randn(1,6);
ins = insinit(avp, 0.01);
ins = insupdate(ins, wvm);
fid = fopen('test.bin', 'wb');
fwrite(fid, [avp; wvm'], 'double');
ins = insupdate(ins, wvm);
psinstypedef(153);
imuerr = imuerrset(1, 1000, 0.1, [50;50;5000]);
davp0 = avpseterr([30;-30;20]*10, 0.1, [1;1;3]*10);
kf = kfinit(ins, davp0, imuerr);
[kf.Phikk_1, Ft] = kffk(ins);
fwrite(fid, kf.xk, 'double');
fwrite(fid, kf.Pxk', 'double');
fwrite(fid, Ft', 'double');
fwrite(fid, kf.Qt', 'double');
fwrite(fid, kf.Hk', 'double');
fwrite(fid, kf.Rk', 'double');
zk = randn(3,1);
fwrite(fid, zk, 'double');
kf = kfupdate(kf, zk);
fclose(fid);
*/
/*	CFileRdWt fin("H:\\ygm2016\\星展惯性\\OCTANS+INS300\\test.bin", -1);
	CVect3 att, vn, pos, wm, vm;
	fin>>att>>vn>>pos>>wm>>vm;
	CSINS ins(CQuat(att), vn, pos);
	ins.Update(&wm, &vm, 1, 0.01);

	CKalman kf(15,3);
	fin>>kf.Xk>>kf.Pk>>kf.Ft>>kf.Qt>>kf.Hk>>kf.Rk>>kf.Zk;
	kf.TimeUpdate(0.01);
	kf.MeasFlagSet(0xff);
	kf.MeasUpdate();
*/
	CQuat q1(CVect3(1,2,3)), q2(CVect3(3,2,2)), q3=q1;
	CVect3 v(1,2,1);
	q1 = q1-v;
	q3 -= v;
	...
	int ii=CVect3(0.0).IsZero();

	CKalman kf(4,2);
	kf.Pk.SetClmVect3(1,1, CVect3(1,2,3));
	kf.Pk.SetRowVect3(1,1, CVect3(1,2,3));
	
	kf.Pk.SetMat3(1,1,CMat3(1,2,3,4,5,6,7,8,9));
	kf.Pk.SetDiag2(1.0, 2.0, 3.0, 4.0);
	double f = norm(diag(kf.Pk));
	kf.Pmax.Set2(1.0, 2.0, 3.0, 3.0);
	kf.Pmin.Set2(2.0, 2.0, 3.0, 4.0);
	kf.PkConstrain();
}
