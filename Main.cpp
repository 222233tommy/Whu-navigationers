...
{
	MyTest();
	CFileRdWt fin("D:\\imugps", 1); //\\data1.txt
	CFileRdWt fins("D:\\ins.bin", 0); //H:\\ygm2016\\精准\\PSINS-CPP\\ins.bin
	CFileRdWt fkf("D:\\kf.bin", 0);
	...
	CCarAHRS kf(ts);
	for(int i=0; i<5000/ts; i++)
	{
		PDOP = 0.0;
		if(!fin.load(1)) break;  // 读数据
		wm = CVect3(&fin.buff[1])*glv.deg*ts;//角速度增量（rad）
		vm = CVect3(&fin.buff[4])*ts;
		gpsVn = CVect3(&fin.buff[10]); PDOP = fin.buff[23];	SatNum = (int)fin.buff[24];
		gpsPos = CVect3(fin.buff[13]*glv.deg, fin.buff[14]*glv.deg, fin.buff[15]);
		gpsYaw = fin.buff[22]*glv.deg;	gpsYaw = gpsYaw>PI ? 2*PI-gpsYaw : -gpsYaw;
